...
function template(type,o={}){
 ...
 if(type==="absent")return base+`आदरणीय पालक, आपला विद्यार्थी ${o.student||""} इयत्ता ${o.className||""} दिनांक ${o.date||""} रोजी अनुपस्थित आहे. कृपया कारण शाळेला कळवावे.`+end;
...
$("saveAttendanceAll").onclick=async()=>{try{for(const row of document.querySelectorAll("#attBody tr"))await saveDoc("attendance",{name:row.dataset.name,className:val("attClass"),date:val("attDate"),status:row.querySelector(".pa").value,time:row.querySelector(".time").value,message:row.querySelector(".note").value,parentMobile:row.dataset.mobile});show("attMsg","Attendance save झाली.")}catch(e){show("attMsg","Error: "+e.message)}};
...
$("generateTemplate").onclick=()=>{$("templateText").value=template(val("templateType"),{student:val("tplStudent")||"विद्यार्थी नाव",className:val("tplClass"),date:today(),teacher:val("tplTeacher")||"वर्ग शिक्षक",subject:"गणित",topic:"धडा",classwork:"आज शिकवलेला भाग",homework:"दिलेला गृहपाठ",text:"सूचना तपशील"});};
$("copyTemplate").onclick=()=>{$("templateText").select();document.execCommand("copy");};
...
$("downloadStudentTemplate").onclick=()=>downloadCsv("student_template.csv",[["name","dob","aadhaar","className","section","rollNo","parentMobile1","parentMobile2","parentMobile3","address"],["कु. विद्यार्थी नाव","2012-01-01","123456789012","9वी","A","1","9876543210","","","पत्ता"]]);
$("uploadStudentCsv").onclick=async()=>{const f=$("studentCsvFile").files[0];if(!f)return show("csvMsg","CSV निवडा");const text=await f.text();const rows=text.trim().split(/\\r?\\n/);const head=rows.shift().split(",").map(x=>x.trim());let c=0;for(const line of rows){const vals=line.split(","),o={};head.forEach((h,i)=>o[h]=vals[i]||"");await saveDoc("students",o);c++}show("csvMsg",c+" विद्यार्थी upload झाले.")};
